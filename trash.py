...
# Текст повідомлення для користувачів, які ще не придбали бота
def get_purchase_message():
    return '''
Привіт!

Якщо ти ще вагаєшся, чи варто придбати бота — зараз саме той момент! Наша база каналів оновлена до майже 3000 каналів, і всі вони ретельно перевіряються. 

Ціна знижена вдвічі, і ти можеш отримати доступ всього за 50 грн! Не втрать шанс скористатися цією вигідною пропозицією!

Придбати можна тут: @vladuslavmen

Чекаємо на тебе! 🌟
'''

def send_purchase_message_to_all_users():
    with sq.connect("Chanelsyoutube_base.db") as con:
        cur = con.cursor()
        cur.execute("SELECT id FROM users WHERE num_buy = 0")
        users = cur.fetchall()

        for user in users:
            user_id = user[0]
            try:
                bot.send_message(user_id, get_purchase_message(), reply_markup=markup)
...
def Start_sms(message):
    if message.from_user.username == 'vladuslavmen':
        bot.send_message(message.from_user.id, "Розсилка пішла!", reply_markup=markup)
        send_purchase_message_to_all_users()
...
Якщо користувач обере дорожчу версію — твій бонус складе 30%.
Тож, навіть якщо співпраця з YouTube-каналами не дасть результатів, ти завжди можеш заробити разом з нами та нашою командою!
            ...
            bot.send_message(
                message.chat.id,
                "У вас немає доступу до каналів. Будь ласка, придбайте доступ, щоб продовжити. Придбати можна у @vladuslavmen",
                reply_markup=markup
            )
        else:
